...
UIE_FRAG_T uie_textcore(float textAlpha, float2 uv, half textureSlot, float2 textCoreUV, float4 vertexColor, half opacity, float sdfScale, bool isDynamicColor, float extraDilate)
{
...
v2f uie_std_vert(appdata_t v)
{
    ...
    float svgSettingIndex = v.settingIndex.x*(255.0f*255.0f) + v.settingIndex.y*255.0f;
    half isArc = v.flags.z > 0.0f ? 1.0f : 0.0f;
    half wFlags = isArc;
    if (v.flags.w > 0.0f)
        wFlags += 2.0f; // Poor man's bitset: dynamic color
    ...
    OUT.colorUVs.xy = isText ? uie_decode_shader_info_texel_pos(v.opacityColorPages.ba, v.ids.w, 4.0f) : OUT.colorUVs.xy;
